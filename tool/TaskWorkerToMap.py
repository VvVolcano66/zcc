...
class WorkerSimulator:
    ...
    def get_available_workers_with_center_info(
        ...
        Args:
            region_id: 区域 ID
            current_time: 当前仿真时间（秒），用于判断工人是否已完成任务

        Returns:
            [(worker_node, wid, lon, lat, center_node), ...]
        """
        available = []
    ...
    def initialize_from_real_data(
        ...
        Args:
            date: 日期
            test_start_hour: 测试开始时间（小时）
            prep_minutes: 准备时间（分钟），默认 5 分钟
            coords: 路网节点坐标
            nodes: 路网节点列表
            partition: 分区字典 {node: region_id}
            centers: 中心点字典 {region_id: center_node}
        """
        import pandas as pd
        import os
    ...
    def calculate_route_with_center(
        ...
        Returns:
            (total_distance, total_time) 总距离和总时间
        """
